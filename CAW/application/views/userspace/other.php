...
<p class="topC0" ><?php echo anchor("userspace/index/$user_id/article/1",'帖子列表');?></P>
<p class="topC0" ><?php echo anchor("userspace/index/$user_id/reply/1",'回复列表');?></P>
<p class="topC0" ><?php echo anchor("userspace/index/$user_id/friend/1",'关注列表');?></P>
...
if($flag=="article"){
	...
	for($i=0;$i<$num;$i++){
		echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp";
		echo "<font style=\"FONT-SIZE:30px BACKGROUND-COLOR:\">";
	...
	echo anchor("userspace/index/$user_id/$flag/1",'首页');
	echo "||";
	if($page<2){
	echo anchor("userspace/index/$user_id/$flag/1",'上一页');
}
else {
	$page2=$page-1;
	echo anchor("userspace/index/$user_id/$flag/$page2",'上一页');
}
	echo"||";
	if($page>$pages-1){
	echo anchor("userspace/index/$user_id/$flag/$pages",'下一页');
}
else{
	$page3=$page+1;
	echo anchor("userspace/index/$user_id/$flag/$page3",'下一页');

}
	echo "||";

	
	echo anchor("userspace/index/$user_id/$flag/$pages",'末页');
	...
	for($i=0;$i<$num;$i++){
		...
		echo anchor("forum/show/$article_id[$i]",$body[$i]);
		$len=substr($friendname[$i]);
		echo "</font>";
		for($a=$len;$a<20;$a++){
			echo "&nbsp;";
...
<?php
echo "<div class=\"img\">";
 echo "<img src=\"".base_url()."/images/user-icon.png\" HEIGHT=\"200\" WIDTH=\"200\"/>";
 echo "</div>";
 echo "<font style=\"FONT-SIZE:20px\">$author[0]</font></br>";
 echo anchor("userspace/care/$user_id",'<button>关注 </button>')."&nbsp;&nbsp;";

 echo anchor("userspace/message/$user_id",'<button>私信 </button>');?>
</DIV>